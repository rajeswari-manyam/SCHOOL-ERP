...
const DAY_BASE =
  "w-[34px] h-[34px] rounded-full flex items-center justify-center text-[13px] font-medium"

// Skeleton cell for loading state
function SkeletonCell() {
  return (
    <div className="h-11 flex items-center justify-center">
      <div className="w-[34px] h-[34px] rounded-full bg-gray-100 animate-pulse" />
    </div>
  )
}
...
export default function AttendanceCalendar({
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const handleClick = () => {
      if (isAbsent) {
        const id = absentMap.get(d)!
        const label = `${MONTH_NAMES[month]} ${d}, ${year}`
        onAbsentClick(id, d, label)
      }
    }
    ...
    if (isToday) {
      dayNode = <div className={cn(DAY_BASE, "bg-sky-500 text-white")}>{d}</div>
    } else if (isAbsent) {
      dayNode = (
        <div
      ...
      dayNode = (
        <div className={cn(DAY_BASE, "bg-indigo-600 text-white")}>{d}</div>
      )
    } else {
      dayNode = (
        <span
          className={cn(
            "text-[13px]",
            isWknd ? "text-gray-400" : "text-gray-400 hover:text-gray-600 transition-colors"
          )}
        >
          {d}
        </span>
