...
const columns = [
  columnHelper.accessor("name", {
    header: "Report Name",
    cell: (info: CellContext<Report, string>) => {
...
const VirtualTable = ({ data, height }: VirtualTableProps) => {
  ...
  return (
    <TableVirtuoso
      ...
      style={{ height }}
      fixedHeaderContent={() =>
        headerGroups.map((headerGroup: HeaderGroup<Report>) => (
          <tr key={headerGroup.id} className="bg-gray-50/80">
            {headerGroup.headers.map((header: Header<Report, unknown>) => (
              <th
                key={header.id}
      ...
      itemContent={(_index: number, row: Row<Report>) => (
        <>
      ...
      components={{
        Table: ({ style, ...props }: { style?: React.CSSProperties; [key: string]: any }) => (
          <table {...props} style={style} className="w-full border-collapse" />
        ),
        TableRow: ({ style, ...props }: { style?: React.CSSProperties; [key: string]: any }) => (
