        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input placeholder="ADF001" className={inputClass(!!errors.code)} {...register("code")} />
              {errors.code && (
                <p className={`text-red-500 ${typography.body.xs} mt-1 flex items-center gap-1`}>
          ...
          <div className="flex gap-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full border-none cursor-pointer transition-colors duration-200 flex-shrink-0 ${taxable ? "bg-[#3525CD]" : "bg-gray-300"}`}
              >
                <span className={`absolute top-[3px] w-[18px] h-[18px] rounded-full bg-white shadow transition-all duration-200 ${taxable ? "left-[23px]" : "left-[3px]"}`} />
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
              {BILLING_CYCLES.map((cycle) => (   // ← from constants
                <button
                  key={cycle}
                  type="button"
          ...
          <div className="sticky bottom-0 bg-white pt-3 border-t border-gray-100 flex flex-col sm:flex-row gap-2 sm:justify-end">
            ...
            <Button type="submit" className="w-full sm:w-auto bg-[#3525CD] hover:bg-[#2a1fb5] text-white">
              Add Fee Head
            </Button>
          </div>
