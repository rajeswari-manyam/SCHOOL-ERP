...
const SchoolsPage = () => {
  ...
  const { importCsv, createSchool } = useSchoolMutations();
...
<AddNewSchoolModal
  ...
  onSuccess={(payload) => {
    console.log("Create school mutation payload:", payload);
    createSchool.mutate(payload, {
      onSuccess: (data) => {
        console.log("Create school mutation success:", data);
        setOpenModal(false);
      },
      onError: (error) => {
        console.error("Create school mutation failed:", error);
      },
    });
  }}
/>
