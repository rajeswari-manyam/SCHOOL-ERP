...
const ConfigTabs = ({ activeTab, onChange }: ConfigTabsProps) => (
  <div className="flex overflow-x-auto rounded-2xl border border-gray-200 bg-white p-1 gap-1 scrollbar-none">
    {TABS.map((t) => (
      <button key={t.value} onClick={() => onChange(t.value)}
        className={cn(
          "shrink-0 whitespace-nowrap px-4 py-2 rounded-xl text-sm font-semibold transition-all",
          activeTab === t.value
