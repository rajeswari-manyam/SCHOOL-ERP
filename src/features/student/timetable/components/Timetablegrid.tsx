...
const TimetableGrid = ({
  ...
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-6 py-4 border-b border-gray-100">
      ...
      <button
        onClick={onPrint}
        className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 text-xs font-semibold text-indigo-600 border border-indigo-200 rounded-xl hover:bg-indigo-50 transition-colors"
      >
        🖨 Print Timetable
      </button>
    </div>
        ...
        if (row.kind === "break") {
          return (
            <div
              ...
              className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-center"
            >
              ...
              {WEEK_DAYS.map((day) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Subject */}
                    <div
                      className={`flex-1 rounded-xl px-3 py-2 ${subjectBg} ${
                        isActive ? "ring-2 ring-indigo-400" : ""
                      }`}
                    >
                      <p
                        className={`font-bold text-sm leading-tight ${subjectColor}`}
                      >
                        {cell.subject}
            ...
            if (row.kind === "break") {
              return (
                ...
                >
                  <td
                    colSpan={8}
                    className="py-2 px-4 text-center text-xs font-bold tracking-widest text-amber-600 uppercase"
                  >
