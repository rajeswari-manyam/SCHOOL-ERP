// teacher/attendance/MarkAttendancePage.tsx
// Full-page version of the former MarkStudentAttendanceModal popup — same
// data/logic, just rendered as a routed page instead of an overlay.
import { useEffect, useMemo, useState } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { ArrowLeft, CheckCircle2, XCircle, Clock, AlertTriangle, ClipboardCheck } from "lucide-react";
...
const MarkAttendancePage = () => {
  ...
  const goBackToAttendance = () => navigate("/teacher/attendance");

  const teacherId = useAuthStore((s) => s.user?.id ?? "");
  const queryClient = useQueryClient();

  // ── Filter state ────────────────────────────────────────────────────────────
  const [classId, setClassId] = useState(routeState.classId ?? "");
  const [sectionId, setSectionId] = useState(routeState.sectionId ?? "");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 px-3 sm:px-6 pt-2 pb-6">
      ...
      <div className="flex items-center gap-1.5 text-xs text-gray-400">
        <button onClick={goBackToAttendance} className="hover:text-gray-600 transition-colors">
          Attendance
        </button>
        <span>›</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-2 px-5 sm:px-6 py-4 border-t border-gray-100 shrink-0">
          <Button variant="ghost" onClick={goBackToAttendance} type="button" size="md">
            Cancel
          </Button>
          <Button
