...
import { STAFF_IMPORT_COLUMNS } from "../types/staffImport.types";
import { IMPORT_FILE_ACCEPT } from "@/utils/importFileKind";

const PREVIEW_ROW_LIMIT = 50;

const ImportStaffExcelPage = () => {
  const navigate = useNavigate();
  const goBackToStaff = () => navigate("/schooladmin/staff");
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400">
        <button type="button" onClick={goBackToStaff} className="hover:text-indigo-600 transition-colors font-medium">
          Staff
        </button>
        <span>›</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between px-5 sm:px-7 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h1 className="text-lg font-bold text-gray-900 leading-tight">Import Staff</h1>
            <p className="text-xs text-gray-400 mt-0.5">
              {importResult ? "Import result" : file ? "Review the file before importing" : "Upload an Excel, CSV, PDF, or Word file containing staff records"}
            </p>
          </div>
          <button onClick={goBackToStaff} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto px-5 sm:px-7 py-6 space-y-5">
          {/* ── Import Result — from real createStaff() calls, not fabricated ── */}
          {importResult ? (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="text-left px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Staff</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {importResult.rows.map((r) => (
                        <tr key={r.row} className={r.status === "Failed" ? "bg-red-50/40" : ""}>
                          ...
                          <td className="px-4 py-2 text-gray-800 font-medium">{r.staffName}</td>
              ...
              {!file && (
                <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-slate-50/50 px-6 py-10 flex flex-col items-center text-center gap-3">
                  ...
                  <p className="text-sm text-gray-500 max-w-sm">
                    Upload the file containing staff records — using the provided Excel template gives the best results.
                  </p>
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  {isDocumentFile && (
                    <div className="flex items-start gap-2.5 text-xs text-gray-600 bg-slate-50 border border-gray-200 rounded-lg px-3.5 py-3">
                      ...
                      <span>
                        ...
                        Excel/CSV imports create staff immediately — this file type will once a document-import backend exists.
                      </span>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <p className="text-xs font-semibold text-gray-600">
                          ...
                          {rowsWithIssues > 0 && (
                            <span className="text-amber-600 font-medium"> · {rowsWithIssues} with issues (won't be submitted)</span>
                          )}
                        </p>
        ...
        <div className="flex items-center justify-between px-5 sm:px-7 py-4 border-t border-gray-100 shrink-0">
          <button onClick={goBackToStaff} className="text-sm font-semibold text-gray-500 hover:text-gray-700 transition-colors">
...
export default ImportStaffExcelPage;
