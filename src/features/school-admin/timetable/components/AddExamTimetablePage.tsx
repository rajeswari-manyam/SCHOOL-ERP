...
import { useExamNameOptions, useBulkCreateExamTimetable, useUpdateExamTimetable } from "../hooks/useTimetable";
...
// Passed via navigate(path, { state }) from TimetablePage — the specific
// entry being edited (undefined/null for a fresh "create" visit), and where
// to return to once the entry is saved or cancelled.
interface AddExamTimetablePageState {
...
const AddExamTimetablePage: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { editData } = (location.state as AddExamTimetablePageState | null) ?? {};
  const goBackToTimetable = () =>
    navigate("/schooladmin/timetable/exams", { state: { activeExamSubTab: "exam-timetable" } });

  const { mutateAsync: bulkCreateExamTimetable, isPending: isCreating } = useBulkCreateExamTimetable();
  const { mutateAsync: updateExamTimetable, isPending: isUpdating } = useUpdateExamTimetable();
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    try {
      setSubmitting(true);
      if (isEditMode && editData) {
        await updateExamTimetable({ id: editData.id, data: examsTimetables[0] });
      } else {
        await bulkCreateExamTimetable({ examsTimetables });
      }
      goBackToTimetable();
    } catch (err: any) {
  ...
  const SF = ({ label, required, loading, value, options, placeholder, disabled, onChange, error }: {
    ...
    <div>
      ...
      <Select value={value} onValueChange={onChange} options={options}
        ...
        className="w-full rounded-xl border border-gray-200 bg-[#EFF4FF] px-4 py-3 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 disabled:opacity-50" />
      {error && <p className="mt-1 text-xs text-red-600">{error}</p>}
    </div>
  );

  const inputCls = (hasErr?: boolean) =>
    `w-full rounded-xl border bg-[#EFF4FF] px-4 py-3 text-sm text-slate-700 outline-none focus:ring-1 transition ${
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400">
        ...
        <span>›</span>
        <span className="text-gray-700 font-semibold">{isEditMode ? "Edit Exam Timetable" : "Add Exam Timetable"}</span>
      </div>
      ...
      <div className="relative w-full overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5 flex-shrink-0">
          ...
          <button onClick={goBackToTimetable} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition">
            <ArrowLeft size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
          <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-gray-200 bg-[#EFF4FF] px-4 py-3 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 resize-none"
              />
            </div>
                          ...
                          <tr key={entry.id} className={blockReason || conflict ? "bg-red-50/40" : "hover:bg-slate-50 transition-colors"}>
                            ...
                            <td className="px-3 py-2 min-w-[140px]">
                              <select
                                ...
                                className={`w-full rounded-lg border px-2 py-1.5 text-xs text-slate-700 outline-none focus:border-indigo-500 ${
                                  errors[`subject_${idx}`] ? "border-red-400 bg-red-50" : "border-gray-200 bg-[#EFF4FF]"
                                }`}
                              >
                            ...
                            <td className="px-3 py-2 min-w-[160px]">
                              <input type="date" value={entry.exam_date}
                                ...
                                className={`w-full rounded-lg border px-2 py-1.5 text-xs text-slate-700 outline-none focus:border-indigo-500 ${
                                  blockReason || conflict || errors[`date_${idx}`] ? "border-red-400 bg-red-50" : "border-gray-200 bg-[#EFF4FF]"
                                }`} />
                              {blockReason && (
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 flex-shrink-0">
            <Button type="button" onClick={goBackToTimetable} variant="outline"
