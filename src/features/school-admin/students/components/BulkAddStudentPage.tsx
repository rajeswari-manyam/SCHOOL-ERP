...
import { ArrowLeft, Plus, Trash2, Loader2, CheckCircle, ChevronDown, ChevronUp, Wand2, ArrowRight, Camera } from "lucide-react";
...
import { getNextAdmissionNumber, formatAdmissionNo } from "../utils/admissionNo";
...
const inputCls = "w-full h-10 px-3 rounded-xl bg-[#EFF4FF] border border-slate-200 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition";
const selectCls = "h-10 rounded-xl bg-[#EFF4FF] border-slate-200";
...
const BulkAddStudentPage = () => {
  ...
  const { students: existingStudents, loading: existingStudentsLoading } = useStudents();
  ...
  // Seeded from existingStudents once loaded, so numbers continue the same
  // sequence as Add Student (e.g. 1 from Add Student, then 2, 3... here)
  ...
  const nextAdmNo = () => {
    ...
    return formatAdmissionNo(admissionNoCounterRef.current);
  };
  ...
  // Seed the counter once existingStudents finishes loading, then back-fill
  // the first row's admission number (only if it hasn't been touched yet).
  useEffect(() => {
    if (seededRef.current || existingStudentsLoading) return;
    seededRef.current = true;
    admissionNoCounterRef.current = getNextAdmissionNumber(existingStudents) - 1;
    setPersonalRows((prev) =>
      prev.length === 1 && !prev[0].admissionNo
        ? [{ ...prev[0], admissionNo: nextAdmNo() }]
        : prev
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [existingStudents, existingStudentsLoading]);
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between px-5 sm:px-7 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h1 className="text-lg font-bold text-gray-900 leading-tight">Bulk Add Students</h1>
            <p className="text-xs text-gray-400 mt-0.5">
              {step === 1 ? "Enter personal details for each student" :
               step === 2 ? "Enter parent & guardian details" : "Students added successfully"}
            </p>
          </div>
          <button onClick={goBackToList} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors shrink-0">
                      ...
                      {expanded && (
                        <div className="px-4 sm:px-6 pb-5 pt-1 bg-slate-50/40 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                          ...
                          <div className="sm:col-span-2 lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <Field label="Class *">
                              {clsLoading ? (
                                <div className="flex items-center gap-2 h-10 px-3 rounded-xl bg-[#EFF4FF] border border-slate-200">
                            ...
                            <Field label="Section *">
                              {!row.classId ? (
                                <div className="flex items-center h-10 px-3 rounded-xl bg-[#EFF4FF] border border-slate-200 text-xs text-gray-400">
                          ...
                          <div className="sm:col-span-2 lg:col-span-4">
                            <Field label="Address">
                              <Textarea
                                ...
                                rows={2}
                                className="rounded-xl bg-[#EFF4FF] border-slate-200 text-sm placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400"
                              />
                            </Field>
                          </div>
                        </div>
        ...
        <div className="flex items-center justify-between px-5 sm:px-7 py-4 border-t border-gray-100 shrink-0">
          <button onClick={goBackToList} className="text-sm font-semibold text-gray-500 hover:text-gray-700 transition-colors">
            {step === 3 ? "Close" : "Cancel"}
          </button>
...
export default BulkAddStudentPage;