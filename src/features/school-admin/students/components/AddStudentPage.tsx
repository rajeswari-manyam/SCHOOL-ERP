...
import { Loader2, ArrowLeft, ArrowRight, MessageCircle, Camera, Trash2 } from "lucide-react";
...
/* ── Field styles ── */
const inputCls = "w-full h-11 px-3.5 rounded-xl bg-[#EFF4FF] border border-slate-200 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition";
...
const PhoneInput = ({
  ...
  <div className={`flex h-11 rounded-xl border overflow-hidden transition focus-within:ring-2 ${
    readOnly
      ...
      : "bg-[#EFF4FF] border-slate-200 focus-within:ring-indigo-300 focus-within:border-indigo-400"
  }`}>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400">
        ...
        <span>›</span>
        <span className="text-gray-700 font-semibold">Add Student</span>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between px-5 sm:px-7 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h1 className="text-lg font-bold text-gray-900 leading-tight">Add New Student</h1>
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <button key={g} type="button" onClick={() => set("gender")(g)}
                        className={`flex-1 h-11 rounded-full text-sm font-semibold border transition-colors ${
                          form.gender === g
                            ...
                            : "bg-[#EFF4FF] text-gray-600 border-gray-300 hover:border-indigo-300 hover:bg-indigo-50"
                        }`}>
                        {g}
                      </button>
                    ))}
              ...
              <Field label="Class" required hasError={e("class")}>
                {classesLoading ? (
                  <div className="flex items-center gap-2 h-11 px-3.5 rounded-xl bg-[#EFF4FF] border border-slate-200">
                  ...
                  <Select value={form.class} onValueChange={set("class")} options={classes} placeholder="Select Class"
                    className="h-11 rounded-xl bg-[#EFF4FF] border-slate-200" />
                )}
              </Field>
              ...
              <Field label="Section" required hasError={e("section")}>
                {!form.class ? (
                  <div className="flex items-center h-11 px-3.5 rounded-xl border border-slate-200 bg-[#EFF4FF] text-xs text-gray-400">
              ...
              <Field label="Blood Group">
                <Select value={form.bloodGroup} onValueChange={set("bloodGroup")} options={BLOOD_GROUPS}
                  placeholder="Select Blood Group" className="h-11 rounded-xl bg-[#EFF4FF] border-slate-200" />
              </Field>
